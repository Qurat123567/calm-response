...
export const generateActionPlan = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<GroqPlan> => {
    ...
    const isOther = data.incidentType === "other" && data.description?.trim();
    const userContent = isOther
      ? `The user describes their situation in their own words:
"""
...
Generate the ordered action plan and ready-to-copy messages tailored to this specific situation now.`
      : `Incident type: ${data.incidentTitle} (${data.incidentType})
