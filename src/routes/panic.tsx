    ...
    return (
      <div className="min-h-screen bg-slate-900 text-white">
        <div className="mx-auto flex min-h-screen max-w-lg flex-col items-center justify-center px-6 text-center">
          ...
          <p className="mt-4 text-lg text-slate-200">Loading your plan…</p>
        </div>
      </div>
    );
  }
      ...
      <div className="min-h-screen bg-slate-900 text-white">
        <div className="mx-auto flex min-h-screen max-w-lg flex-col items-center justify-center px-6 text-center">
          ...
          <button
            ...
            className="mt-8 w-full rounded-2xl bg-white px-6 py-4 text-lg font-bold text-slate-900"
          >
            Choose incident
          </button>
          <Link to="/" className="mt-4 text-sm text-slate-400 underline">
            Back home
          </Link>
        </div>
      </div>
      ...
      <div className="min-h-screen bg-slate-900 text-white">
        <div className="mx-auto max-w-lg px-6 py-8">
          <button
            ...
            className="inline-flex items-center gap-2 text-base text-slate-200"
          >
            <ArrowLeft className="h-5 w-5" /> Back
          </button>
          <h2 className="mt-6 text-2xl font-bold">{m.title}</h2>
          <p className="mt-4 whitespace-pre-wrap text-lg leading-relaxed text-slate-50">
            {m.body}
          </p>
          <button
            onClick={() => navigator.clipboard?.writeText(m.body)}
            className="mt-8 w-full rounded-2xl bg-white px-6 py-5 text-lg font-bold text-slate-900"
          >
            Copy message
          </button>
        </div>
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-white">
      <div className="mx-auto max-w-lg px-6 py-8">
        ...
        <p className="mt-2 text-lg text-slate-200">Breathe. One step at a time.</p>
        ...
        <div className="mt-6 rounded-2xl bg-slate-800/60 p-5">
          ...
          <div className="mt-3 h-3 w-full overflow-hidden rounded-full bg-slate-700/60">
            <div
          ...
          {nextStep && (
            <div className="mt-4 rounded-xl bg-slate-700/60 p-4">
          ...
          <Link
            ...
            className="mt-4 block text-center text-sm text-slate-200 underline"
          >
            Open full checklist
          </Link>
        </div>
        ...
        <div className="mt-3 space-y-3">
          {plan.messages.map((m, i) => (
            <button
              ...
              className="w-full rounded-2xl bg-white px-5 py-5 text-left text-lg font-bold text-slate-900 shadow-lg"
            >
              {m.title}
            </button>
          ))}
